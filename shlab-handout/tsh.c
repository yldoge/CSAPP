...
void eval(char *cmdline) {
    ...
    if (!builtin_cmd(argv)) { // if not built-in command
        ...
        if ((pid = fork()) < 0) {
            ...
            setpgid(0, 0); // give the child an independent process group
...
void sigchld_handler(int sig) {
    ...
    /*
     * WNOHANG: "but doesn't wait for any other currently running children to terminate"
     * WUNTRACED: also return when a child job stops because of signal
     */
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
        if (WIFEXITED(status)) { // child exit normally
            ...
            // change the job's state to ST (stopped)
            struct job_t* job = getjobpid(jobs, pid);
            job->state = ST;
...
void sigint_handler(int sig) {
    ...
    if (pid == 0) // no foreground job running
        return;
